    ...
    class FieldSetHandler
      def initialize(params:, allowed: {}, required: [])
        ...
        allowed.each_value do |option|
          raise InternalArgumentError.new(allowed: "must be a Enum") unless option.is_a?(Dry::Types::Enum)
        end
        @allowed = allowed
        raise InternalArgumentError.new(allowed: "must be an Array") unless required.is_a?(Array)
        @required = required
      end
      ...
      def parse_option(type, option)
        ...
        raise ExternalArgumentError.new(field_set: "invalid field: <#{option}> for type: #{type}")
      end

      def check_required_field_set_types(field_set)
        return field_set if (required - field_set.keys).empty?
        raise ExternalArgumentError.new(field_set: "missing required field_set parameter")
      end

      def raise_invalid_field_option(type)
        return if allowed&.key?(type)
        raise OptionNotAllowedError.new(field_set: "field_set for type: #{type} not allowed")
      end

      def raise_missing_fields_param
        return if required.nil? || required.empty?
        raise ExternalArgumentError.new(field_set: "missing required fields options")
      end
